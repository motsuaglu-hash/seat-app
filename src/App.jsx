      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="flex border-b border-slate-200 bg-slate-50 sticky top-0 z-10">
            ...
            <SidebarTab active={activeTab === 'manual'} onClick={() => setActiveTab('manual')} icon={BookOpen}>
              マニュアル
            </SidebarTab>
          </div>
...
**席替えサポート**は、クラスの座席配置を直感的に作成・調整できるWebツールです。

## 基本的な考え方
- **名簿**と**配慮事項**（隣にしない生徒など）を登録します
- 「⚡ 自動配置」でたたき台を作ります
- 座席を**ドラッグ＆ドロップ**で手動調整して仕上げます
- すべてのデータはこの端末（ブラウザ）内だけに保存されます（外部送信なし）

## 画面の構成
- **中央**：座席グリッド（黒板を上にした教室のレイアウト）
- **左（設定パネル）**：座席／名簿／配慮／保存／マニュアルの5タブ
- **下部**：配慮事項の警告一覧
    `,
    setup: `
# 初期設定の手順
...
## 1. クラス名の入力
画面上部の入力欄に「3年A組」のように入力します（JSON書き出し時のファイル名にも使われます）。

## 2. 座席のサイズ
左パネル「座席」タブで**行数・列数**を設定します（各1〜20）。
使用可能席数は自動計算され、名簿人数と比較されます。

## 3. 使用不可席の設定
「座席削除モード」をONにして、座席グリッド上の席をタップすると
その席が「使用不可」（黒板の死角・机がない場所など）になります。
もう一度タップで解除できます。「使用不可席をリセット」で一括解除も可能です。
    `,
    roster: `
# 名簿登録

## 1件ずつ登録
左パネル「名簿」タブで、出席番号・性別・氏名（必須）・ふりがなを入力し「追加」します。

## CSV風テキストで一括登録
「CSV風テキストで一括登録」を開き、以下の形式で複数行貼り付けます。
\`\`\`
1,青木太郎,あおきたろう,男
2,伊藤花子,いとうはなこ,女
\`\`\`
出席番号,氏名,ふりがな,性別 の順（カンマまたはタブ区切り、氏名は必須）。

## 編集・削除
名簿の各行の✎で編集、🗑で削除できます（削除すると座席や配慮事項からも自動的に外れます）。
    `,
    considerations: `
# 配慮事項の設定

左パネル「配慮」タブで生徒を選択すると設定できます（座席上の生徒をタップして選ぶことも可能です）。

## 位置の希望
- 前方希望 / 後方希望（同時には設定不可）
- 左側希望 / 右側希望（同時には設定不可）
- 教卓近く希望
- 🔒固定席：自動配置・移動の対象外にし、現在位置を維持します

## 隣にしない生徒 / 近くにしない生徒（8マス以内）
チェックした生徒とは、自動配置時になるべく離すよう配置します。
片方だけに設定しても、自動配置・警告チェックの両方で相互に考慮されます。
    `,
    auto: `
# 自動配置の使い方
...
## 自動配置の仕組み
- 🔒固定席の生徒は現在位置のままロックされます
- 配慮事項（希望・NG指定）が多い生徒ほど優先的に配置されます
- 「隣にしない」「近くにしない」を最優先で避け、次に前後左右・教卓近くの希望を反映します
- ランダム性があるため、同じ設定でも実行のたびに結果が変わることがあります

## 実行後の確認
- 下部の警告欄で配慮事項の違反や未配置がないか確認してください
- 気になる箇所はドラッグで手動調整できます
    `,
    manual_op: `
# 手動調整の方法

## 座席の入替
- 生徒のいる座席をドラッグし、別の座席にドロップすると**入れ替わります**
- iPad・スマホでは、座席を少し**長押し**してからドラッグしてください（誤操作防止のため）

## 固定席について
- 🔒固定席の生徒はドラッグ移動できません（先に配慮タブで固定を解除してください）
- 固定席の生徒がいる席へ他の生徒を移動しようとした場合も、入替は行われません

## 元に戻す
- ヘッダーの「↩ 元に戻す」で直前の操作を取り消せます（最大30件）

## 座席配置の初期化
左パネル「座席」タブの「座席配置を初期化」で、全員を座席から外せます（名簿は消えません）。
    `,
    warnings: `
# 警告の見方

画面下部に配慮事項の違反・未解決事項が一覧表示されます。

## 主な警告の種類
- **定員不足**（エラー）：名簿人数に対して使用可能席が足りません
- **使用不可席への配置**（エラー）：使用不可席に生徒が配置されています
- **未配置**（警告）：座席に配置されていない生徒がいます
- **隣同士になっている**（エラー）：「隣にしない」指定の生徒同士が隣接しています
- **近くに配置されている**（警告）：「近くにしない」指定の生徒同士が8マス以内にいます
- **希望と異なる位置**（警告）：前方／後方／左右／教卓近くの希望に沿っていません

赤（エラー）は特に優先して解消し、黄（警告）は状況に応じて調整してください。
座席グリッド上でも、警告のある生徒の席はオレンジ色の枠で強調表示されます。
    `,
    view: `
# 生徒用・教卓用の表示切替

ヘッダーの「生徒用」「教卓用」ボタンで、編集画面とは別の表示専用画面を開けます。

## 生徒用
黒板を上にした、教室で見た通りの向きの座席表です。掲示・共有向けです。

## 教卓用
教員が教卓から見た向き（上下左右反転）で表示されます。出欠確認などに使えます。

編集画面に戻るには、各画面左上の「← 編集に戻る」をクリックします。
    `,
    print: `
# 印刷方法

生徒用・教卓用いずれかの画面（またはヘッダーの「🖨 印刷」）から印刷できます。
編集画面から直接「印刷」を押した場合は、自動的に生徒用表示に切り替わってから印刷されます。

## 用紙設定
A4横向き・1ページに収まるレイアウトで最適化されています。
ブラウザの印刷ダイアログで用紙サイズを「A4」、向きを「横」にしてください。
    `,
    save: `
# 保存と引き継ぎ

## 自動保存
編集内容はブラウザの**localStorage**に自動保存されます（画面上部に保存状況が表示されます）。
同じ端末・同じブラウザで再度開くと、続きから編集できます。
プライベート（シークレット）モードでは自動保存が無効になる場合があります。

## 別端末への引き継ぎ
左パネル「保存」タブの「JSON書き出し」でファイルを保存し、
別の端末で「JSON読み込み」から読み込むと、同じデータで編集を続けられます。
定期的なバックアップとしても活用してください。

## 全データ初期化
「保存」タブの「全データ初期化」で、名簿・座席・配慮事項などすべてを消去できます
（確認ダイアログが出ます。元に戻せないため注意してください）。

## 🔒 プライバシーについて
このアプリは外部サーバーにデータを送信しません。名簿・座席情報はすべて
お使いの端末内（localStorageまたは書き出したJSONファイル）にのみ保存されます。
    `,
    trouble: `
# よくあるトラブル

## 生徒をドラッグできない
→ その生徒が🔒固定席になっている可能性があります。配慮タブで固定を解除してください。

## 自動配置しても配慮事項が守られない
→ 使用不可席や固定席の配置によっては、すべての希望を満たす配置が
　物理的に不可能な場合があります。下部の警告欄で内容を確認し、手動で調整してください。

## 名簿人数より座席が少ないと言われる
→ 「座席」タブで行数・列数を増やすか、使用不可席を減らしてください。

## データが消えた／別端末で開いたら空だった
→ localStorageは端末・ブラウザごとに保存されるため共有されません。
　「保存」タブの「JSON読み込み」で書き出しておいたファイルから復元してください。

## 印刷がレイアウト崩れする
→ ブラウザの印刷設定で用紙を「A4」・向きを「横」にしているか確認してください。
    `,
  };
  ...
  return (
    <div>
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs leading-relaxed text-slate-700 whitespace-pre-wrap">
        {content[section] || ''}
      </div>
    </div>
  );
